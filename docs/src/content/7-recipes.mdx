...
## Media State
<code>useWindowQuery</code> can be used to handle responsive layouts with React state. For example, take a grid with one row and three columns:
...
const Component = () => {
    ...
    return(
        <Grid mx={isSmall ? 10 : 20} bgColor="#feccfe" textCenter>
            <Row>
                <Col>🌲</Col>
                {isSmall && <Col>🍄</Col>}
                {!isSmall && <Col>🌻</Col>}
