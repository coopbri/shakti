...
- <code>breakpoint</code> accepts a pixel value to test against. You could use the Shakti default breakpoints outlined above, or use your own pixel breakpoint here.

It may make more sense in code. For example, if you'd like to check if the window width is less than or equal to the Shakti small (<code>sm</code>) breakpoint, you would have
...
<code>true</code> is returned if the test passes (in this case, if the window width is equal to or below the small breakpoint), and <code>false</code> is returned otherwise. <code>undefined</code> is returned if the test parameters are invalid.
